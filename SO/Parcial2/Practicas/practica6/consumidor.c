...
int main(int argc, char **argv){
    int i = 0, i2 = 0, consumos = 0,consumo = 0, status[3];
    char mensaje[15];
    char *rutas[10] = {"/tmp/llamada1.txt", "/tmp/llamada2.txt", "/tmp/llamada3.txt",
                    "/tmp/llamada4.txt", "/tmp/llamada5.txt",
                    "/tmp/mensaje1.txt", "/tmp/mensaje2.txt", "/tmp/mensaje3.txt",
                    "/tmp/mensaje4.txt", "/tmp/mensaje5.txt"};
    FILE *archivo;
    ...
    for(i2 = 0; i2 < 3; i2++){
        if((pid = fork()) == -1){
            ...
            while(consumos < 33334){
                if((apuSeccion[0]->producto == 10000) && (apuSeccion[1]->producto == 10000)
                    && (apuSeccion[2]->producto == 10000) && (apuSeccion[3]->producto == 10000)
                    && (apuSeccion[4]->producto == 10000) && (apuSeccion[5]->producto == 10000)
                    && (apuSeccion[6]->producto == 10000) && (apuSeccion[7]->producto == 10000)
                    && (apuSeccion[8]->producto == 10000) && (apuSeccion[9]->producto == 10000)){
                    exit(0);
                }else{
                ...
                for(i = 0; i < 10; i++){
                    ...
                    if(apuSeccion[i]->bandera == 1){
                        ...
                        printf("%d - Consumidor %d, consumio:%s en la seccion critica %d\n", apuSeccion[i]->producto, i2+1, apuSeccion[i]->mensaje, i);
